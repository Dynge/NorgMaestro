...
public sealed class DidChangeHandlerTests
{
    ...
    [Fact]
    public void ShouldIgnoreInvalidRangeWhenEndBeforeStart()
    {
        string tempDir = Directory.CreateTempSubdirectory("norgmaestro-didchange-invalid").FullName;
        try
        {
            string sourcePath = Path.Combine(tempDir, "source.norg");
            File.WriteAllText(sourcePath, "@document.meta\ntitle: Old\n@end\n");
            ...
            state.Documents[sourceUri].Metadata.Title!.Name.Should().Be("Old");
            writer.WriteCount.Should().BeGreaterThan(0);
        }
        finally
        {
